...
export class PessoaComponent implements OnInit {
  ...
  ngOnInit(): void {
    if(!this.session.user.perfil.estrategico){
      ...
      this.pessoas.find(productIdFromRoute).subscribe(data => {
        this.data$ = data;
      });
    }
  }
  ...
  salvar(){
    this.cadformarq.controls.pessoa_id.patchValue(this.data$.id);
    this.arquivos.pessoas(this.cadformarq.value).subscribe(data => {
      if(data == 1){
        this.cadformarq.reset();
        this.pessoas.find(this.data$.id).subscribe(data => {
          this.data$ = data;
        });
