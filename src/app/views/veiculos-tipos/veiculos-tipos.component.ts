...
export class VeiculosTiposComponent implements OnInit {
  ...
  salvar(){
    ...
    if(this.cadform.value.id){
      this.veiculostipos.put(this.cadform.value).subscribe(data => {
        //console.log(data);
        if(data == 1){
          this.cadform.reset();
          this.getData();
          this.showToast('Informações salvas!','Tipo de veículo editado com sucesso.',1);
        }else{
          this.showToast('Erro!','Erro ao editar, tento novamente mais tarde!',2);
        }
      });
    }else{
      this.veiculostipos.post(this.cadform.value).subscribe(data => {
        //console.log(data);
        if(data == 1){
          this.cadform.reset();
          this.getData();
          this.showToast('Informações salvas!','Tipo de veículo cadastrado com sucesso.',1);
        }else{
  ...
  confirm(id: number){
    this.veiculostipos.delete(id).subscribe(data =>{
      //console.log(data);
      if(data == 1){
        this.cadform.reset();
        this.getData();
        this.showToast('Exclusão realizada!','Tipo de veículo excluido com sucesso.',1);
      }else{
