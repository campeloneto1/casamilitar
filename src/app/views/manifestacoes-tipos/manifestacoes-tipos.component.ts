...
@Component({
  selector: 'app-manifestacoes-tipos',
  templateUrl: './manifestacoes-tipos.component.html',
  styleUrls: ['./manifestacoes-tipos.component.css']
})
export class ManifestacoesTiposComponent implements OnInit {
  ...
  salvar(){
    ...
    if(this.cadform.value.id){
      this.manifestacoestipos.put(this.cadform.value).subscribe(data => {
        //console.log(data);
        if(data == 1){
          this.cadform.reset();
          this.getData();
          this.showToast('Informações salvas!','Tipo de manifestação editada com sucesso.',1);
        }else{
      ...
      this.manifestacoestipos.post(this.cadform.value).subscribe(data => {
        //console.log(data);
        if(data == 1){
          this.cadform.reset();
          this.getData();
          this.showToast('Informações salvas!','Tipo de manifestação cadastrada com sucesso.',1);
        }else{
  ...
  confirm(id: number){
    this.manifestacoestipos.delete(id).subscribe(data =>{
      //console.log(data);
      if(data == 1){
        this.cadform.reset();
        this.getData();
        this.showToast('Exclusão realizada!','Tipo de manifestação excluida com sucesso.',1);
      }else{
