...
export class InfluenciasComponent implements OnInit {
  ...
  salvar(){
    ...
    if(this.cadform.value.id){
      this.influencias.put(this.cadform.value).subscribe(data => {
        //console.log(data);
        if(data == 1){
          this.cadform.reset();
          this.getData();
          this.showToast('Informações salvas!','Marca editada com sucesso.',1);
        }else{
      ...
      this.influencias.post(this.cadform.value).subscribe(data => {
        //console.log(data);
        if(data == 1){
          this.cadform.reset();
          this.getData();
          this.showToast('Informações salvas!','Marca cadastrada com sucesso.',1);
        }else{
  ...
  confirm(id: number){
    this.influencias.delete(id).subscribe(data =>{
      //console.log(data);
      if(data == 1){
        this.cadform.reset();
        this.getData();
        this.showToast('Exclusão realizada!','Marca excluida com sucesso.',1);
      }else{
