...
export class CoresComponent implements OnInit {
  ...
  salvar(){
    ...
    if(this.cadform.value.id){
      this.cores.put(this.cadform.value).subscribe(data => {
        //console.log(data);
        if(data == 1){
          this.cadform.reset();
          this.getData();
          this.showToast('Informações salvas!','Cor editada com sucesso.',1);
        }else{
      ...
      this.cores.post(this.cadform.value).subscribe(data => {
        //console.log(data);
        if(data == 1){
          this.cadform.reset();
          this.getData();
          this.showToast('Informações salvas!','Cor cadastrada com sucesso.',1);
        }else{
  ...
  confirm(id: number){
    this.cores.delete(id).subscribe(data =>{
      //console.log(data);
      if(data == 1){
        this.cadform.reset();
        this.getData();
        this.showToast('Exclusão realizada!','Cor excluida com sucesso.',1);
      }else{
