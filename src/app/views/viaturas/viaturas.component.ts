...
export class ViaturasComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if(!this.session.user.perfil.viaturas){
      ...
      this.viaturas.index().subscribe(data => {
        this.data$ = data;
        this.temp$ = this.data$;
      });
    }
  ...
  filterdata() {
    if(this.textfilter){
      ...
      const temp = this.data$.filter((d: any) => {
        return d.placa.toLowerCase().indexOf(val) !== -1 || d.renavam.toLowerCase().indexOf(val) !== -1 || d.chassi.toLowerCase().indexOf(val) !== -1 || d.marca.nome.toLowerCase().indexOf(val) !== -1 || d.modelo.nome.toLowerCase().indexOf(val) !== -1 || d.veiculo_tipo.nome.toLowerCase().indexOf(val) !== -1 || !val;
      });
  ...
  salvar(){
    ...
    if(this.cadform.value.id){
      this.viaturas.put(this.cadform.value).subscribe(data => {
        //console.log(data);
        if(data == 1){
          this.cadform.reset();
          this.getData();
          this.showToast('Informações salvas!','Viatura editada com sucesso.',1);
        }else{
      ...
      this.viaturas.post(this.cadform.value).subscribe(data => {
        //console.log(data);
        if(data == 1){
          this.cadform.reset();
          this.getData();
          this.showToast('Informações salvas!','Viatura cadastrada com sucesso.',1);
        }else{
  ...
  confirm(id: number){
    this.viaturas.delete(id).subscribe(data =>{
      //console.log(data);
      if(data == 1){
        this.cadform.reset();
        this.getData();
        this.showToast('Exclusão realizada!','Viatura excluida com sucesso.',1);
      }else{
