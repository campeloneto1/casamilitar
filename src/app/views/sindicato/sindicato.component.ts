...
export class SindicatoComponent implements OnInit {
  ...
  constructor(private route: ActivatedRoute, 
    ...
    private sindicato: SindicatosService,
  ...
  ngOnInit(): void {
    if(!this.session.user.perfil.estrategico){
      ...
      this.sindicato.find(productIdFromRoute).subscribe(data => {
        this.data$ = data;
      });
    }
  }
  ...
  salvar(){
    this.cadformarq.controls.sindicato_id.patchValue(this.data$.id);
    this.arquivos.sindicatos(this.cadformarq.value).subscribe(data => {
      if(data == 1){
        this.cadformarq.reset();
        this.sindicato.find(this.data$.id).subscribe(data => {
          this.data$ = data;
        });
